...
export default function CreateGiftModal({ isOpen, onClose, onSuccess }: CreateGiftModalProps) {
  ...
  const loadNextCode = async () => {
    const response = await apiClient.getNextGiftCode();
  ...
  const handleReplaceImage = () => {
    const fileInput = document.getElementById('image-upload-create') as HTMLInputElement;
    fileInput?.click();
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await apiClient.createGift(giftData);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-green-600 text-white px-6 py-4 flex justify-between items-center rounded-t-lg">
          <h2 className="text-xl font-semibold">Créer un cadeau</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 bg-blue-50 dark:bg-blue-900/20 px-3 py-2 rounded-md">
              Code auto-généré: <span className="font-semibold text-blue-700 dark:text-blue-300">{nextCode || 'Chargement...'}</span>
            </p>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            ...
            >
              {isSubmitting ? 'Création...' : 'Créer le cadeau'}
            </button>
          </div>
        </form>
      </div>
